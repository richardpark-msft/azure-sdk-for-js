...
export async function sample1ReceiveMessagesViaHandlerNoSessions() {
  const queueConsumerClient: QueueReceiverClient = new QueueReceiverClient(
    "connection-string",
    "queue-name"
  );

  // receiveMode passed as part of the `consume` call
  const closeableThing: Closeable = await queueConsumerClient.consume(
    "PeekLock",
    {
...
export async function sample3SendMessages() {
  const queueProducerClient: QueueSenderClient = new QueueSenderClient(
    "connection-string",
    "queue-name"
  );
