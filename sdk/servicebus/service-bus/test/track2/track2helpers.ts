...
import { QueueReceiverClient } from "../../src/track2/queueReceiverClient";

import { env } from "process";

import { EnvVarKeys } from "../utils/envVarUtils";

import { QueueSenderClient } from "../../src/track2/queueSenderClient";
...
export async function demoPeekLock(): Promise<void> {
  const consumerClient = new QueueReceiverClient(
...
export interface TestClients {
  consumer: QueueReceiverClient;
  producer: QueueSenderClient;
  close(): Promise<void>;
}
...
export async function createTestClients(mode: "sessions" | "nosessions"): Promise<TestClients> {
  ...
  const consumer = new QueueReceiverClient(
    env[EnvVarKeys.SERVICEBUS_CONNECTION_STRING]!,
    queueName
  );
  const producer = new QueueSenderClient(
    env[EnvVarKeys.SERVICEBUS_CONNECTION_STRING]!,
    queueName
  );
...
export async function drainQueue(queueClient: QueueReceiverClient): Promise<void> {
