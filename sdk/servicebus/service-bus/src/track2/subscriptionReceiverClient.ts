...
import { SubscriptionReceiverClientOptions, SettleableContext, Message, FetchOptions, FetchResult, PlainContext, ReceiverHandlers, Closeable, SessionMessage, SessionContext, PeekedMessage } from './models';
import { SessionReceiver, Receiver } from '../receiver';
import { ReceiveMode, ServiceBusMessage, MessagingError } from '..';
import { SubscriptionClient } from "../subscriptionClient";
import { peekBySequenceNumber, peek } from './utils/peekHelpers';

export class SubscriptionReceiverClient {
  private _sbClient: ServiceBusClient;
  private _subscriptionClient: SubscriptionClient;

  constructor(topicConnectionString: string, subscriptionName: string, options?: SubscriptionReceiverClientOptions);    // #1
  constructor(serviceBusConnectionString: string, topicName: string, subscriptionName: string, options?: SubscriptionReceiverClientOptions);    // #2
  constructor(topicConnectionStringOrServiceBusConnectionString1: string, subscriptionNameOrTopicNameOrOptions2: string, optionsOrSubscriptionName3?: string | SubscriptionReceiverClientOptions, options4?: SubscriptionReceiverClientOptions) {
    if (typeof optionsOrSubscriptionName3 === "string") {
      // #2
      const serviceBusConnectionString = topicConnectionStringOrServiceBusConnectionString1;
      const topicName = subscriptionNameOrTopicNameOrOptions2;
      ...
      const subscriptionName: string = subscriptionNameOrTopicNameOrOptions2;
      const options: undefined | SubscriptionReceiverClientOptions = optionsOrSubscriptionName3;
  ...
  fetch(
    ...
    if (sessionId != null) {
      receiver = this._subscriptionClient.createReceiver(receiveMode, { sessionId });
    } else {
      receiver = this._subscriptionClient.createReceiver(receiveMode);
    }
  ...
  async peekWithoutLock(
    ...
    const clientContext = this._subscriptionClient["_context"];
  ...
  private _createReceiveAndDeleteReceiver(
    ...
    const receiver = this._subscriptionClient.createReceiver(ReceiveMode.receiveAndDelete);
  ...
  private _createPeekLockReceiver(
    ...
    const receiver = this._subscriptionClient.createReceiver(ReceiveMode.peekLock);
  ...
  private _createPeekLockReceiverForSession(
    ...
    const receiver: SessionReceiver = this._subscriptionClient.createReceiver(ReceiveMode.peekLock, {
      sessionId: sessionId
    });
